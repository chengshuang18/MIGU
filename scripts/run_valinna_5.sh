...
for seed in "${seeds[@]}"; do
  for lr in "${lrs[@]}"; do
    ...
    mkdir -p output/t5_large/valinna/sequential/order_5/logs/${seed}
    echo "bash scripts/t5_large/valinna/order_5.sh ${cluster} ${ini_threshold} ${cluster_constructure_method} ${activation_combined} ${seed} ${lr}> output/t5_large/valinna/sequential/order_5/logs/${seed}/train_and_infer_${lr}.log  2>&1"
    bash scripts/t5_large/valinna/order_5.sh ${cluster} ${ini_threshold} ${cluster_constructure_method} ${activation_combined} ${seed} ${lr}> output/t5_large/valinna/sequential/order_5/logs/${seed}/train_and_infer_${lr}.log  2>&1
  done
done
